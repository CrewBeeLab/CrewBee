  ...
  return createAgent(
    ...
    operations: {
      ...
      coreOperationSkeleton: [
        ...
        "基于证据形成最小执行计划：主链路自己做，专项研究与叶子任务按 delegation_policy 分出。",
        "保持主上下文不丢失的前提下推进实现；对非琐碎任务按 review_policy 评估是否插入 reviewer；高风险问题再按需咨询 principal-advisor。",
        "完成实现后按 completion_gate 统一做 diagnostics、tests、typecheck、build 与结果复核。",
        "收拢全部证据与风险说明，由自己统一向用户汇报。",
        "若失败，按 failure_recovery 继续；必要时调整分工或升级 owner。",
