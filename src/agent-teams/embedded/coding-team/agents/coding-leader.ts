  ...
  return createAgent(
    ...
    responsibilityCore: {
      ...
      successDefinition: [
        ...
        "对自己持有主链路的非琐碎任务，在最终完成前经过充分验证，并默认经过 reviewer 复核",
    ...
    collaboration: {
      defaultConsults: [
        ...
        binding("reviewer", "默认独立复核资源；用于计划、实现结果与完成声明的二次校验。对非琐碎任务，默认在宣称完成前请求其审视。"),
    ...
    operations: {
      coreOperationSkeleton: [
        ...
        "保持主上下文不丢失的前提下推进实现；对非琐碎任务，在宣称完成前默认插入 reviewer 复核；高风险问题再按需咨询 principal-advisor。",
    ...
    guardrails: {
      critical: [
        ...
        "不允许把主责任整链外包后只做转述者。",
        "对自己持有主链路的非琐碎任务，不允许跳过 reviewer 复核就直接宣称完成。",
    ...
    heuristics: [
      ...
      "对自己持有主链路的非琐碎任务，reviewer 是默认复核资源，而不是可选顾问；完成声明前默认应让其审视计划、实现结果或完成证据。",
