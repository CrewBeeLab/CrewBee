...
test("upsertCrewBeePluginEntry migrates project-local entries to the canonical user-level entry", () => {
  ...
  const result = upsertCrewBeePluginEntry(config, "file:///tmp/user/.cache/opencode/crewbee/node_modules/crewbee/opencode-plugin.mjs");

  assert.equal(result.changed, true);
  assert.deepEqual(result.migratedEntries, ["file:///tmp/project/node_modules/crewbee/opencode-plugin.mjs"]);
  assert.deepEqual(config.plugin, [
    "foreign-plugin",
    "file:///tmp/user/.cache/opencode/crewbee/node_modules/crewbee/opencode-plugin.mjs",
  ]);
});
...
test("upsertCrewBeePluginEntry preserves non-string plugin entries", () => {
  ...
  upsertCrewBeePluginEntry(config, "file:///tmp/user/.cache/opencode/crewbee/node_modules/crewbee/opencode-plugin.mjs");
